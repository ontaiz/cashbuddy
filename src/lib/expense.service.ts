...
export async function createExpense(
  ...
  const { data: expense, error } = await supabase.from("expenses").insert(insertData).select().single();

  // Handle database errors
  if (error) {
    throw new ExpenseServiceError("Failed to create expense", "DATABASE_ERROR", error);
  }

  // Handle case where no data is returned (shouldn't happen with .single())
  if (!expense) {
    throw new ExpenseServiceError("No expense data returned after insert", "NO_DATA_RETURNED");
  }
...
export async function getPaginatedExpenses(
  ...
  let query = supabase.from("expenses").select("*", { count: "exact" }).eq("user_id", userId);
  ...
  // Handle database errors
  if (error) {
    throw new ExpenseServiceError("Failed to retrieve expenses", "DATABASE_ERROR", error);
  }

  // Handle case where no data is returned
  if (!data) {
    throw new ExpenseServiceError("No expenses data returned", "NO_DATA_RETURNED");
  }
...
export async function getExpenseById(
  ...
  if (error) {
    ...
    throw new ExpenseServiceError("Failed to retrieve expense", "DATABASE_ERROR", error);
  }
...
export async function updateExpense(
  ...
  if (error) {
    ...
    // Other database errors
    throw new ExpenseServiceError("Failed to update expense", "DATABASE_ERROR", error);
  }
...
export async function deleteExpense(
  ...
  // Handle database errors
  if (error) {
    throw new ExpenseServiceError("Failed to delete expense", "DATABASE_ERROR", error);
  }

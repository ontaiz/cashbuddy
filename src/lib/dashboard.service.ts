 ...
 */
export async function getDashboardData(supabase: SupabaseClient, userId: string): Promise<DashboardDataDto> {
  ...
  const [totalExpensesResult, currentMonthExpensesResult, topExpensesResult, monthlySummaryResult] = await Promise.all([
    ...
    supabase.from("expenses").select("amount, date").eq("user_id", userId).order("date", { ascending: false }),
  ]);

  // Handle errors from parallel queries
  if (totalExpensesResult.error) {
    throw new DashboardServiceError("Failed to retrieve total expenses", "DATABASE_ERROR", totalExpensesResult.error);
  }
  ...
  if (topExpensesResult.error) {
    throw new DashboardServiceError("Failed to retrieve top expenses", "DATABASE_ERROR", topExpensesResult.error);
  }
  ...
  const totalExpenses = (totalExpensesResult.data || []).reduce((sum, expense) => sum + (expense.amount || 0), 0);
